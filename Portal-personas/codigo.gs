...
function guardarSolicitud(d) {
  ...
  if (d.tipoFormulario === 'RECLUTAMIENTO') {
    ...
    headers = ["FOLIO", "TOKEN", "ESTADO", "FECHA REGISTRO", "SOLICITANTE", "EMAIL SOLICITANTE", "NOMBRE TRABAJADOR", "RUT", "FECHA INGRESO", "CENTRO DE COSTO", "CARGO", "JEFE DIRECTO", "DESDE", "HASTA", "MOTIVO", "OBSERVACIONES", "FECHA FIRMA JEFE", "GERENTE AREA", "FECHA FIRMA GERENTE"];
    fila = [folioFinal, t, "PENDIENTE", new Date(), d.nombreSolicitante || "N/A", d.emailSolicitante || "N/A", d.nombre1 || "N/A", d.rut || "N/A", d.fechaIngresoTrabajador || "N/A", d.permCc || "N/A", d.permCargo || "N/A", d.permJefe || "N/A", d.permFechaInicio || "N/A", d.permFechaFin || "N/A", d.permMotivo || "N/A", d.permObservacion || "N/A", d.permFechaFirmaJefe || "N/A", d.permGerente || "N/A", d.permFechaFirmaGerente || "N/A"];
    ...
    const baseH = ["FOLIO", "TOKEN", "ESTADO", "FECHA REGISTRO", "SOLICITANTE", "EMAIL SOLICITANTE", "NOMBRE TRABAJADOR", "RUT", "CORREO PERSONAL", "CARGO/FUNCIÓN", "PROYECTO/ÁREA", "FECHA INGRESO", "FECHA TÉRMINO", "MOTIVO DESVINCULACIÓN", "FORMULARIO SALIDA", "PUNTAJE PROMEDIO"];
    const baseF = [folioFinal, t, "PENDIENTE", new Date(), d.nombreSolicitante || "N/A", d.emailSolicitante || "N/A", d.nombre1 || "N/A", d.rut || "N/A", d.correoPersonal || "N/A", d.evaCargo || "N/A", d.evaArea || "N/A", d.evaFechaIngreso || "N/A", d.evaFTermino || "N/A", d.evaMotivoLista || "N/A", d.evaFormularioSalida || "N/A", d.evaPuntuacion || "0.0"];
    const punH = [], punF = [];
    for (let i = 1; i <= 10; i++) { punH.push("P" + i); punF.push(d["evaP" + i] || "0"); }
    for (let i = 1; i <= 10; i++) { punH.push("P" + i + " OBS"); punF.push(d["evaP" + i + "Obs"] || ""); }
    const cualH = ["3.1 EVAL. GENERAL", "3.2 RECONTRATARÍA", "3.3 MEJORAS", "3.4 SITUAC. CRÍTICAS", "RECOMIENDA RECONTRATAR"];
    const cualF = [d.evaCual1 || "N/A", d.evaCual2 || "N/A", d.evaCual3 || "N/A", d.evaCual4 || "N/A", d.evaRecomienda || "N/A"];
    const devNames = ["EPP", "HERRAMIENTAS", "CREDENCIALES", "DOCUMENTACIÓN", "DISPOSITIVOS", "ROPA TRABAJO"];
    const devH = [], devF = [];
    for (let i = 1; i <= 6; i++) {
      devH.push("DEV " + devNames[i - 1] + " ESTADO", "DEV " + devNames[i - 1] + " OBS");
      devF.push(d["evaDev" + i] || "N/A", d["evaDev" + i + "Obs"] || "");
    }
    const firmH = ["EVALUADOR", "CARGO EVALUADOR", "VISACIÓN GERENTE", "VISACIÓN RR.HH."];
    const firmF = [d.evaEvaluador || "N/A", d.evaCargoEvaluador || "N/A", d.evaVisaGerente || "N/A", d.evaVisaRRHH || "N/A"];
    headers = baseH.concat(punH, cualH, devH, firmH);
    fila = baseF.concat(punF, cualF, devF, firmF);
  }
